...
export default function HeaderDropdown(props: Props) {
    ...
    return <div className="flex flex-col grow" onClick={handleClick}>
        ...
        <div className={`lg:absolute inner-shadow bg-white overflow-hidden transition-all top-[calc(100%+1rem)] ${isOpen ? 'max-h-screen' : 'max-h-0'}`}>
            <div className="lg:p-4 shadow-inner bg-gray-50" onFocus={handleFocus} onBlur={handleBlur}>
                {props.children}
            </div>
        </div>
    </div>;
