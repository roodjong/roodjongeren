...
export async function fetchPrivacybeleid(): Promise<PrivacybeleidContent> {
    const response = await backend.get<StrapiResponse<PrivacybeleidContent>>('/privacybeleid');
...
export async function fetchConfidantsPage(): Promise<ConfidantsPageContent> {
    const response = await backend.get<StrapiResponse<ConfidantsPageContent>>('/confidants-page', {
        params: {
            populate: {
                banner: {
                    fields: ['url']
